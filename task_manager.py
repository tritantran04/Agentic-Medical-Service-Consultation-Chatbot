...
class AgentTaskManager(InMemoryTaskManager):
    def __init__(self, agent: ServiceAgent, notification_sender_auth: PushNotificationSenderAuth):
        super().__init__()
        self.agent = agent
    ...
    def _validate_request(
        ...
        if not utils.are_modalities_compatible(
            task_send_params.acceptedOutputModes, ServiceAgent.SUPPORTED_CONTENT_TYPES
        ):
            logger.warning(
                "Unsupported output mode. Received %s, Support %s",
                task_send_params.acceptedOutputModes,
                ServiceAgent.SUPPORTED_CONTENT_TYPES,
            )
    ...
    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        ...
        # [SỬA LỖI THẬT] Bản trước gọi set_push_notification_info() TRƯỚC upsert_task().
        # set_push_notification_info() tra self.tasks[task_id] và raise ValueError nếu
        # task chưa tồn tại — nhưng task chỉ được TẠO trong upsert_task(). Kết quả: mọi
        # request tasks/send có kèm pushNotification cho 1 task_id mới đều crash bằng
        # "ValueError: Task not found for ...", lỗi này không được try/except nên bay
        # thẳng lên server.py, trả về lỗi 500 mơ hồ thay vì lỗi rõ ràng như ý đồ code.
        # Đã kiểm chứng bằng test thực tế. Cách sửa: gọi upsert_task() TRƯỚC — đúng thứ tự
        # mà on_send_task_subscribe() bên dưới vốn đã làm đúng.
